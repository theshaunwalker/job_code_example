...
class SuspendSubscriptionHandler extends CommandHandler
{
    /**
    ...
    public function handle(SuspendSubscriptionCommand $command)
    {
        $subscription = $this->subscriptionSourceRepository->load($command->getSubscriptionId());

        $subscription->suspend($command->getReason());
